...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "avengermoviedb" because\n';
  ...
  $wnd.__gwt_activeModules['avengermoviedb'] = {
    'moduleName' : 'avengermoviedb',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/avengermoviedb.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['avengermoviedb'].superdevmode = true;

  var expectedSuffix = '/avengermoviedb.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + avengermoviedb + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:avengermoviedb';
  var devModeSessionKey = '__gwtDevModeSession:avengermoviedb';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/avengermoviedb/avengermoviedb.nocache.js';
  ...
  $wnd.__gwt_activeModules['avengermoviedb'].canRedirect = true;
  $wnd.__gwt_activeModules['avengermoviedb'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING avengermoviedb';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/avengermoviedb?user.agent=' + ua + '&_callback=' + callback;
